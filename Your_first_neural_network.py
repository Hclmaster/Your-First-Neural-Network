...
# 这里的意思是: 它是为了展示前10天的，然后每一天中它都记录了24h
# 所以它展示的是每一天的24h的情况。
...
# season-4, weathersit-4, mnth-12, hr-24(0), weekday-7(0)
dummy_fields = ['season', 'weathersit', 'mnth', 'hr', 'weekday']
for each in dummy_fields:
    # get_dummies是将拥有不同值的变量转换为0/1数值
    dummies = pd.get_dummies(rides[each], prefix=each, drop_first=False)
    # axis:需要合并链接的轴，0是行，1是列
    # concat不会去重,这里就是单纯的进行轴连接
...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        #print("在init里面的shape为: "+str(self.weights_input_to_hidden.shape))
        #print("在init里面的shape为: "+str(self.weights_hidden_to_output.shape))
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            # print("---->hidden_outputs shape: "+str(hidden_outputs.shape))

            # TODO: Output layer - Replace these values with your calculations.
            final_inputs = np.dot(hidden_outputs, self.weights_hidden_to_output) # signals into final output layer
            # print("---->final_inputs shape: "+str(final_inputs.shape))
            # final_outputs的activation function是f(x)=x?
            final_outputs = final_inputs # signals from final output layer
            # print("---->final_outputs shape: "+str(final_outputs.shape))
            ...
            # print("----->hidden_error shape: "+str(hidden_error.shape))
            
            hidden_error_term = hidden_error * hidden_outputs * (1 - hidden_outputs)
            # print("----->hidden_error_term shape: "+str(hidden_error_term))

            # Weight step (input to hidden)
            delta_weights_i_h += X[:,None] * hidden_error_term
            # print("------->delta_weights_i_h shape: "+str(delta_weights_i_h.shape))
            
            # print("这有什么问题？ output_error_term.shape, hidden_outputs.shape: "+str(output_error_term.shape)+", "+str(hidden_outputs))
            
            # Weight step (hidden to output)
            delta_weights_h_o += (hidden_outputs * output_error_term).reshape(self.weights_hidden_to_output.shape)
            # print("------->delta_weights_h_o shape:"+str(delta_weights_h_o.shape))
            

        # TODO: Update the weights - Replace these values with your calculations.
        self.weights_hidden_to_output += self.lr * delta_weights_h_o / n_records # update hidden-to-output weights with gradient descent step
        self.weights_input_to_hidden += self.lr * delta_weights_i_h / n_records # update input-to-hidden weights with gradient descent step
        # print("---->weights_hidden_to_output: "+str(self.weights_hidden_to_output))
        # print("---->weights_input_to_hidden: "+str(self.weights_input_to_hidden))

    # run这个函数是用来进行预测的，所以只需要前传的部分
...
class TestMethods(unittest.TestCase):
    ...
    def test_train(self):
        ...
        # 传入参数分别为: input_nodes, hidden_nodes, output_nodes, learning_rate
        network = NeuralNetwork(3, 2, 1, 0.5)
        network.weights_input_to_hidden = test_w_i_h.copy()
        network.weights_hidden_to_output = test_w_h_o.copy()
        # print("weights_input_to_hidden: "+str(network.weights_input_to_hidden.shape))
        # print("weights_hidden_to_output: "+str(network.weights_hidden_to_output.shape))
